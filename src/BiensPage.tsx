...
// Convertit une ligne Supabase en objet Bien local.
// Les champs loyer/charges/dépenses viendront des autres tables plus tard.
function fromDb(row: any): Bien {
  return {
    id:       row.id,
    nom:      row.nom,
    adresse:  row.adresse   ?? "",
    type:     (row.type     ?? "autre") as TypeBien,
    regime:   (row.regime_fiscal ?? "micro-foncier") as Regime,
    statut:   "vacant",   // sera alimenté par la table locataires
    loyer_hc: 0,          // sera alimenté par la table recettes
    charges:  0,
    depenses: 0,
  }
}
      ...
      {tab === "infos" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div style={{ background: C.wh, borderRadius: 14, padding: "18px 20px", border: `1px solid ${C.br}` }}>
            ...
            {loue && bien.locataire ? (
              <>
                <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 8 }}>👤 {bien.locataire}</div>
...
function AddBienModal({
  ...
  const handleSave = async () => {
    ...
    if (!user) {
      setError("Connexion requise. Ajoutez l'authentification à l'étape suivante.")
      setSaving(false)
      return
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FieldInput label="Nom du bien *" placeholder="Ex. Appartement Gambetta" value={form.nom} onChange={set("nom")} />
        <FieldInput label="Adresse" placeholder="12 rue de la Paix, 75001 Paris" value={form.adresse} onChange={set("adresse")} />
        ...
        <FieldInput label="Locataire actuel" placeholder="Nom du locataire (vide = vacant)" value={form.locataire} onChange={set("locataire")} />

        <div style={{ height: 1, background: C.br, margin: "4px 0 16px" }} />
        <div style={{ fontSize: 11, fontWeight: 700, color: C.gl, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 14 }}>
          Finances mensuelles
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          <FieldInput label="Loyer HC (€)" type="number" placeholder="0" value={form.loyer_hc} onChange={set("loyer_hc")} />
          <FieldInput label="Charges (€)"  type="number" placeholder="0" value={form.charges}  onChange={set("charges")} />
          <FieldInput label="Dépenses (€)" type="number" placeholder="0" value={form.depenses} onChange={set("depenses")} />
        </div>

        {(form.loyer_hc || form.charges || form.depenses) && (
          <div style={{ background: cashflowPreview >= 0 ? C.gp : C.rp, borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 13, color: C.tm }}>Cash-flow estimé / mois</span>
            <span style={{ fontSize: 20, fontWeight: 900, color: cashflowPreview >= 0 ? C.g : C.rd }}>
              {cashflowPreview >= 0 ? "+" : ""}{euro(cashflowPreview)}
            </span>
          </div>
        )}

        {error && (
          <div style={{ background: C.rp, color: C.rd, borderRadius: 8, padding: "9px 12px", fontSize: 13, marginBottom: 12, fontWeight: 600 }}>
            {error}
          </div>
        )}
...
export default function BiensPage() {
  ...
  async function loadBiens() {
    ...
    setBiens((data ?? []).map(fromDb))
  }

  const nbLoues       = biens.filter(b => b.statut === "loue").length
  const totalLoyer    = biens.reduce((s, b) => s + b.loyer_hc, 0)
  const totalCashflow = biens.reduce((s, b) => s + b.loyer_hc + b.charges - b.depenses, 0)

  if (selected) {
    return <BienDetail bien={selected} onBack={() => setSelected(null)} />
  }
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: C.tm, marginTop: 3 }}>
            {loading
              ? "Chargement…"
              : `${biens.length} bien${biens.length !== 1 ? "s" : ""} · ${nbLoues} loué${nbLoues !== 1 ? "s" : ""}`}
          </div>
        </div>
        <button
      ...
      {!loading && biens.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 22 }}>
          {[
            ...
            { label: "Cash-flow / mois", value: (totalCashflow >= 0 ? "+" : "") + euro(totalCashflow), color: totalCashflow >= 0 ? C.g : C.rd },
          ].map(s => (
      ...
      {!loading && !error && biens.length === 0 && (
        <div style={{ background: C.wh, borderRadius: 16, padding: "60px 20px", textAlign: "center", border: `1px solid ${C.br}` }}>
          ...
          <div style={{ color: C.tm, fontSize: 13, marginBottom: 20 }}>Ajoutez votre premier bien immobilier.</div>
          <button
            onClick={() => setShowAdd(true)}
