...
function DepenseLigne({ d, onEdit, onDelete }: {
  d: Depense
  onEdit: () => void
  onDelete: () => void
}) {
  ...
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", padding:"11px 0", borderBottom:`1px solid ${C.br}`, gap:12 }}>
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {d.description && <div style={{ fontSize:13, color:C.tx, fontWeight:500 }}>{d.description}</div>}
        <div style={{ fontSize:11, color:C.tm, marginTop:2 }}>📅 {formatDate(d.date)}</div>
      </div>
      <div style={{ textAlign:"right", flexShrink:0 }}>
        <div style={{ fontWeight:800, fontSize:15, color:C.rd, whiteSpace:"nowrap", marginBottom:6 }}>
          −{euro(d.montant)}
        </div>
...
// ── Modal dépense (ajout + modification) ──────────────────
...
function DepenseModal({
  ...
  const [form, setForm] = useState<FormState>({
    ...
    categorie:   initialValues?.categorie            ?? "copropriete",
