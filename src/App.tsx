...
const CAT_CHG = ["Crédit immobilier – capital","Crédit immobilier – intérêts","Assurance emprunteur","Taxe foncière","Charges de copropriété","Assurance PNO","Travaux & réparations","Petit matériel","Fournitures & courses","Frais de gestion / conciergerie","Honoraires comptable","Honoraires notaire / avocat","Eau / EDF / Gaz","Internet / Téléphone","Entretien & nettoyage","Annonces & marketing","Mobilier & équipement","Parties communes","Autre charge"];
  ...
  return(
    <>
      <Card>
        ...
        {fil.length===0?<div style={{textAlign:"center",color:C.tm,padding:"24px 0",fontSize:13}}>Aucun{isC?"e charge":" revenu"} pour {an}</div>
          :[...fil].sort((a,b)=>b.date.localeCompare(a.date)).map(t=>(
            <div key={t.id} className="trow" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"9px 4px",borderBottom:`1px solid ${C.br}`,borderRadius:6,transition:"background .15s",gap:8}}>
              <div style={{flex:1}}>
